...
def test_fileio_dsc_binaryfile():  # fail on binary file
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/misc/null.sys.dsc')


def test_fileio_dsc_emptyfile():  # fail on empty file
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/empty.dsc')


def test_fileio_dsc_ab_format_error1():  # fail on network line
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_network1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_network2.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_missing_network.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_duplicate_network.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_missplaced_network.dsc')


def test_fileio_dsc_ab_format_error2():  # fail on no node section
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_only_network.dsc')


def test_fileio_dsc_ab_format_error3():  # fail on node section
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_node1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_duplicate_node.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_type1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_type2.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_type3.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_node_value1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_duplicate_node_value.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_node_bad_num_values.dsc')


def test_fileio_dsc_ab_format_error4():  # fail on node probability errors
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob_node.dsc')


def test_fileio_dsc_ab_format_error5():  # node errors
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob_cond_node1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob_cond_node2.dsc')


def test_fileio_dsc_ab_format_error6():  # prob errors
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob2.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_prob3.dsc')


def test_fileio_dsc_ab_format_error7():  # prob errors
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob1.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob2.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob3.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob4.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob5.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob6.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob7.dsc')
    with pytest.raises(FileFormatError):
        read_bn(TESTDATA_DIR + '/dsc/ab_bad_cond_prob8.dsc')
...
def test_fileio_dsc_asia():
    bn = read_bn(TESTDATA_DIR + '/asia/asia.dsc')
    assert isinstance(bn, BN)
    assert bn.free_params == 18


def test_fileio_dsc_alarm():
    bn = read_bn(TESTDATA_DIR + '/alarm/alarm.dsc')
    assert isinstance(bn, BN)
    assert bn.free_params == 509


def test_fileio_dsc_pathfinder():
    bn = read_bn(TESTDATA_DIR + '/pathfinder/pathfinder.dsc')
    assert isinstance(bn, BN)
...
def test_fileio_dsc_write_not_found():  # fail on write to non-existent path
    bn = read_bn(TESTDATA_DIR + '/dsc/ab.dsc')
    with pytest.raises(FileNotFoundError):
        write_bn(bn, TESTDATA_DIR + '/nonexistent/ab.dsc')
