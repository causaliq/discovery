...
def values_dataset_sachs_c():  # 10m rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/sachs_c.xdsl')
...
def values_dataset_covid_c():  # 10M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/covid_c.xdsl')
    data = bn.generate_cases(10000000).astype('float32')
    data = Pandas(df=data)
...
def values_dataset_building_c():  # 1M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/building_c.xdsl')
...
def values_dataset_ecoli70_c():  # 1M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/ecoli70_c.xdsl')
    data = bn.generate_cases(1000000).astype('float32')
    data = Pandas(df=data)
...
def values_dataset_niab_c():  # 1M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/magic-niab_c.xdsl')
    data = bn.generate_cases(1000000).astype('float32')
    data = Pandas(df=data)
...
def values_dataset_irri_c():  # 1M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/magic-irri_c.xdsl')
    data = bn.generate_cases(1000000).astype('float32')
    data = Pandas(df=data)
...
def values_dataset_arth150_c():  # 1M rows
    bn = read_bn(EXPTS_DIR + '/bn/xdsl/arth150_c.xdsl')
    data = bn.generate_cases(1000000).astype('float32')
    data = Pandas(df=data)
