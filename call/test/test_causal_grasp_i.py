...
# Integration tests of causal-learn GRASP algorithm
...
def test_causal_grasp_ab_3_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/ab_3.csv',
                      dstype="categorical")
    graph, trace = causal_learn(algorithm="grasp", data=data)
    print(f"\n\nGraph learnt by GRASP from 3 rows of AB:\n{graph}")
...
@requires_causal_learn
def test_causal_grasp_ab_strong_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/ab_strong.csv',
                      dstype="categorical")
    graph, trace = causal_learn(algorithm="grasp", data=data)
    print(f"\n\nGraph learnt by GRASP from ab_strong:\n{graph}")
...
@requires_causal_learn
def test_causal_grasp_ab_random_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/ab_random.csv',
                      dstype="categorical")
    graph, trace = causal_learn(algorithm="grasp", data=data)
    print(f"\n\nGraph learnt by GRASP from ab_random:\n{graph}")
...
@requires_causal_learn
def test_causal_grasp_abc_5_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/abc_5.csv',
                      dstype="categorical")
    graph, trace = causal_learn(algorithm="grasp", data=data)
    print(f"\n\nGraph learnt by GRASP from abc_5:\n{graph}")
...
@requires_causal_learn
def test_causal_grasp_abc_36_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/abc_36.csv',
                      dstype="categorical")
    graph, trace = causal_learn(algorithm="grasp", data=data)
    print(f"\n\nGraph learnt by GRASP from abc_36:\n{graph}")
...
# Asia, 100 rows
@requires_causal_learn
def test_causal_grasp_asia_100_ok():
    ...
    assert graph.nodes == ["asia", "bronc", "dysp", "either", "lung", "smoke",
                           "tub", "xray"]
    assert graph.number_components() == 4
    assert graph.edges == {
    ...
    assert values_same(trace.trace["delta/score"][-1], -267.137363, sf=9)


# Asia, 1K rows
@requires_causal_learn
def test_causal_grasp_asia_1k_ok():
    data = NumPy.read(filename=TESTDATA_DIR +
                      '/experiments/datasets/asia.data.gz',
    ...
    assert values_same(trace.trace["delta/score"][-1], -2268.997866, sf=10)
...
@requires_causal_learn
def test_causal_grasp_child_1k_timeout_error():
    data = NumPy.read(filename=TESTDATA_DIR +
                      '/experiments/datasets/child.data.gz',
                      dstype="categorical", N=1000)
    with pytest.raises(RuntimeError):
        causal_learn(algorithm="grasp", data=data, maxtime=1,
                     context={"in": "asia.dsc", "id": "grasp/child/1k"})
...
@requires_causal_learn
def test_causal_grasp_xyz_10_ok():
...
@requires_causal_learn
def test_causal_grasp_gauss_100_ok():
