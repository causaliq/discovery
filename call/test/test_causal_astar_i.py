...
# Integration tests of causal-learn exact astar score-based algorithms
...
def test_causal_astar_categorical_value_error():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/ab_3.csv',
                      dstype="categorical")
    with pytest.raises(ValueError):
        causal_learn(algorithm="astar", data=data)
...
def test_causal_astar_xy_3_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/xy_3.csv',
                      dstype="continuous")
    graph, trace = causal_learn(algorithm="astar", data=data,
...
@requires_causal_learn
def test_causal_astar_xyz_10_ok():
...
@requires_causal_learn
def test_causal_astar_gauss_100_ok():
...
@requires_causal_learn
def test_causal_astar_gauss_1k_ok():
    data = NumPy.read(filename=TESTDATA_DIR + '/simple/gauss.data.gz',
